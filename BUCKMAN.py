#!/usr/bin/env python
# IRC Class
# Developed by Andross Mecum in Python 3
# https://github.com/AndrossMecum/BUCKMAN
...
# Identity
nickname = 'i_steal_code'
username = 'how2editsauce'
realname = 'i am a maniacal retard that can't edit source files'
...
class IRC(object):
    ...
    def event_connect(self):
        if self.nickserv:
            self.identify(self.nickname, self.nickserv)
        if self.oper_passwd:
    ...
    def event_join(self, nick, chan):
        if chan == '#dev':
           self.sendmsg(chan, 'Welcome to #dev, f{nick}')
    ...
    def event_kick(self, nick, chan, kicked):
        if kicked == self.nickname and chan == self.channel:
            ...
            self.join(self.channel, self.key)
        if chan == '#dev':
           self.sendmsg(chan, 'lol fukn owned faggot')

    def event_message(self, nick, chan, msg):
        if msg == 'h':
            self.sendmsg(chan, 'h')
        elif msg.startswith('.test'):
           self.sendmsg(chan, f'IT\nPRIVMSG {chan} :WORKS')
        elif msg == '.die':
             self.quit()
        elif msg.startswith == '.join':
        elif msg.startswith('.mimic'):
             mimic = msg.strip('.mimic ')
             self.sendmsg(chan, f'{mimic}')
